...
static rt_err_t drv_pwm_get(TIM_HandleTypeDef *htim, struct rt_pwm_configuration *configuration)
{
    ...
    /* ---------- 关键修改：根据定时器实例选择正确的时钟源，不乘2 ---------- */
    if (htim->Instance == TIM1 || htim->Instance == TIM8 || htim->Instance == TIM12 ||
        htim->Instance == TIM13 || htim->Instance == TIM14 || htim->Instance == TIM15 ||
        htim->Instance == TIM16 || htim->Instance == TIM17)
    {
        tim_clock = tim_clock = HAL_RCC_GetPCLK2Freq() * 2;      // 高级/通用定时器挂在 APB2 上，直接取 PCLK2
    }
    else
    {
        tim_clock = tim_clock = HAL_RCC_GetPCLK1Freq() * 2;     // 其他定时器挂在 APB1 上，直接取 PCLK1
    }
    /* ---------------------------------------------------------------- */

    /* 处理定时器自身的时钟分频（TIMx_CR1 中的 CKD 位） */
...
static rt_err_t drv_pwm_set(TIM_HandleTypeDef *htim, struct rt_pwm_configuration *configuration)
{
    ...
    rt_uint64_t tim_clock = 0, psc;
    rt_uint32_t channel = 0x04 * (configuration->channel - 1);
    rt_uint32_t pclk1 = HAL_RCC_GetPCLK1Freq();
    rt_uint32_t pclk2 = HAL_RCC_GetPCLK2Freq();

    /* ---------- 关键修改：与 get 保持一致的时钟选择逻辑 ---------- */
    if (htim->Instance == TIM1 || htim->Instance == TIM8 || htim->Instance == TIM12 ||
        htim->Instance == TIM13 || htim->Instance == TIM14 || htim->Instance == TIM15 ||
        htim->Instance == TIM16 || htim->Instance == TIM17)
    {
        tim_clock = tim_clock = HAL_RCC_GetPCLK2Freq() * 2;
    }
    else
    {
        tim_clock = tim_clock = HAL_RCC_GetPCLK1Freq() * 2;
    }
    /* ------------------------------------------------------------ */
